...
class LoadEnglishGermanDataset():
    ...
    def load_train_val_test_data(self, train_file_paths, val_file_paths, test_file_paths):
        train_data = self.data_process(train_file_paths)  #长度为 29000 的列表, 每个元素为元组  (de_sentence_tokens, en_sentence_tokens)
        val_data = self.data_process(val_file_paths)  #长度为 1014, 表示有 1014 个用于 validation 的德英句子对
        test_data = self.data_process(test_file_paths)  #长度为 1000, 表示有 1014 个用于 validation 的德英句子对
        train_iter = DataLoader(train_data, batch_size=self.batch_size,
                                shuffle=True, collate_fn=self.generate_batch)  # collate_fn：数据加载器的一个参数，用于指定如何将单个样本组合成一个批次
    ...
    def generate_batch(self, data_batch):
        ...
        for (de_item, en_item) in data_batch:  # 开始对一个batch中的每一个样本进行处理。
            ...
            en = torch.cat([torch.tensor([self.BOS_IDX]), en_item, torch.tensor([self.EOS_IDX])], dim=0)  # 对于输出的德语加入起止符
            en_batch.append(en)
        # 以最长的序列为标准进行填充
