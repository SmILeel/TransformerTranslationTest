...
def train_model(config):
    ...
    logging.info("############初始化模型############")
    translation_model = TranslationModel(src_vocab_size=len(data_loader.de_vocab),  # 输入德语的字典大小
                                         tgt_vocab_size=len(data_loader.en_vocab),  # 输出英语的字典大小
                                         d_model=config.d_model,  # 学得 embedding 的大小
                                         nhead=config.num_head,  # head 的个数
                                         num_encoder_layers=config.num_encoder_layers,  # encoder 的层数
                                         num_decoder_layers=config.num_decoder_layers,  # decoder 的层数
                                         dim_feedforward=config.dim_feedforward,  # 全连接层面中间层的 维度
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (src, tgt) in enumerate(train_iter):
            ...
            tgt_input = tgt[:-1, :]  # 解码部分的输入, [tgt_len-1, batch_size], 注意这里的操作导致了 tgt_len-1 即不需要去学习结尾的，主要是保持形状
            ...
            logits = translation_model(
                ...
                src_mask=src_mask,  # Encoder的注意力Mask输入，这部分其实对于Encoder来说是没有用的, 全 False shape(src_len, src_len)
                tgt_mask=tgt_mask,  # 形状为 (tgt_len-1, tgt_len-1), 防止解码器中的注意力从右往左流动
                # Decoder的注意力Mask输入，用于掩盖当前position之后的position [tgt_len,tgt_len]
                src_key_padding_mask=src_padding_mask,  # 用于mask掉Encoder的Token序列中的padding部分, shape=(batch, src_len)
                tgt_key_padding_mask=tgt_padding_mask,  # 用于mask掉Decoder的Token序列中的padding部分, shape=(batch, tgt_len-1)
            ...
            optimizer.zero_grad()
            tgt_out = tgt[1:, :]  # 解码部分的真实值  shape: [tgt_len-1, batch_size]
            loss = loss_fn(logits.reshape(-1, logits.shape[-1]), tgt_out.reshape(-1))  # 采用的是交叉熵损失函数
    ...
    torch.save({'iter_loss': iter_loss, 'epoch_loss': epoch_loss,
               ...
               os.path.join(config.model_save_dir, 'metrics_record.pkl'))
