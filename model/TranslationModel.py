...
# from torch.nn import Transformer as MyTransformer  # 这句话导致实际上并没有使用自定义的 transformer 模型
...
class TranslationModel(nn.Module):
    def __init__(self, src_vocab_size, tgt_vocab_size,
        ...
        self.classification = nn.Linear(d_model, tgt_vocab_size)  # tgt_vocab_size=10838
    ...
    def decoder(self, tgt, memory):
        tgt_embed = self.tgt_token_embedding(tgt)  # [tgt_len, batch_size, embed_dim]
        tgt_embed = self.pos_embedding(tgt_embed)  # [tgt_len, batch_size, embed_dim] decoder 中也添加了 position encoding
