...
class MyTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        ...
        self.norm1 = nn.LayerNorm(d_model)  # 一个特征被进行 layernorm 操作
...
class MyTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        ...
        :param d_model:         d_k = d_v = d_model/nhead = 64, 模型中向量的维度，论文默认值为 512
        :param nhead:           多头注意力机制中多头的数量，论文默认为值 8
        :param dim_feedforward: 全连接中向量的维度，论文默认值为 2048 = 512*4
        :param dropout:         丢弃率，论文中的默认值为 0.1    
        """
        ...
        self.activation = nn.ReLU()  # 采用的 ReLU 激活函数
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None,
        ...
        :return:
        """
        tgt2 = self.self_attn(tgt, tgt, tgt,  # [tgt_len,batch_size, embed_dim] 用 decoder 作为 q，取 memory 为 k v
                              attn_mask=tgt_mask,
...
class MyTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None,
        ...
        for mod in self.layers:  # 这里的layers就是N层解码层堆叠起来的
            output = mod(output, memory,  # 随着层数加深，尽管 output 的 embedding 在发生变化，但他们取的都是编码器输出的 memory
                         tgt_mask=tgt_mask,
...
def multi_head_attention_forward(query,  # [tgt_len,batch_size, embed_dim]
    ...
    q = q_proj(query)
    #  q:[tgt_len,batch_size, embed_dim] x q_proj:[embed_dim,kdim * num_heads] = [tgt_len,batch_size,kdim * num_heads]

    k = k_proj(key)
    # k:[src_len, batch_size, embed_dim] x k_proj:[embed_dim, kdim * num_heads] = [src_len, batch_size, kdim * num_heads]

    v = v_proj(value)
    # v:[src_len, batch_size, embed_dim] x v_proj:[embed_dim, vdim * num_heads] = [src_len, batch_size, vdim * num_heads]
    if is_print_shape:
        print("" + "=" * 80)
        print("进入多头注意力计算:")
        print(
    ...
    scaling = float(head_dim) ** -0.5   # 即文中的 sqrt(d_k)
    ...
    attn_output_weights = torch.bmm(q, k.transpose(1, 2))  # torch.bmm 将批中的每对矩阵相乘，返回一个新的三维张量，形状为 (batch_size, n, p)。
    ...
    if attn_mask is not None:  # 当 attn_mask 元素都为 Inf 加上之后不会产生变化
    ...
    Z = out_proj(attn_output)  # Fig.2 右边的图顶上的 linear 层
